...
class TestActivationCodeView:
    def test_activate_user_success(self, api_client, user, activation_code):
        ...
        """
        response = api_client.post("/auth/activate-user/", {"activation_code": activation_code.code})
    ...
    def test_activate_invalid_code(self, api_client):
        ...
        """
        # Use a valid-length code that does not exist.
        response = api_client.post("/auth/activate-user/", {"activation_code": "INVLD12"})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_activate_expired_code(self, api_client, user, db):
        ...
        expired_code = ActivationCode.objects.create(
            user=user,
            code="EXPRD12",  # valid 8-character code
            created_at=timezone.now() - timedelta(minutes=11)  # expired: valid for 10 minutes
        )
        response = api_client.post("/auth/activate-user/", {"activation_code": expired_code.code})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_throttling(self, api_client, settings):
        ...
        """
        settings.REST_FRAMEWORK['DEFAULT_THROTTLE_RATES'] = {'activation': '2/minute'}

        test_code = "INVLD12"  # valid-length code that does not exist.

        # Make two failed activation attempts.
        api_client.post("/auth/activate-user/", {"activation_code": test_code})
        api_client.post("/auth/activate-user/", {"activation_code": test_code})

        # The third attempt should now be throttled.
        response = api_client.post("/auth/activate-user/", {"activation_code": test_code})
